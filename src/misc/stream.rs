...
impl BinaryStream {
	...
	pub fn write_string(&mut self, data: &str) {
		self.write_u16(data.len() as u16, Endianness::BigEndian);
...
impl BinaryStream {
	...
	pub fn read(&mut self, size: usize) -> Option<Vec<u8>> {
		if self.position + size > self.buffer.len() { return None }
	...
	pub fn read_u16(&mut self, endianness: Endianness) -> Option<u16> {
		if self.position + 2 > self.buffer.len() { return None }
	...
	pub fn read_u24(&mut self, endianness: Endianness) -> Option<u24> {
		if self.position + 3 > self.buffer.len() { return None }
        let bytes: [u8; 4] = [self.buffer[self.position], self.buffer[self.position + 1], self.buffer[self.position + 2], 0];
        self.position += 3;
	...
	pub fn read_u32(&mut self, endianness: Endianness) -> Option<u32> {
		if self.position + 4 > self.buffer.len() { return None }
        let bytes: [u8; 4] = [self.buffer[self.position], self.buffer[self.position + 1], self.buffer[self.position + 2], self.buffer[self.position + 3]];
        self.position += 4;
	...
	pub fn read_u64(&mut self, endianness: Endianness) -> Option<u64> {
		if self.position + 8 > self.buffer.len() { return None }
        let bytes: [u8; 8] = [self.buffer[self.position], self.buffer[self.position + 1], self.buffer[self.position + 2], self.buffer[self.position + 3], self.buffer[self.position + 4], self.buffer[self.position + 5], self.buffer[self.position + 6], self.buffer[self.position + 7]];
        self.position += 8;
	...
	pub fn read_string(&mut self) -> Option<String> {
		let length = self.read_u16(Endianness::BigEndian)?;
        if self.position + length as usize > self.buffer.len() { return None }
		let string = std::str::from_utf8(&self.buffer[self.position..self.position + length as usize]);
	...
	pub fn read_i8(&mut self) -> Option<i8> {
		if self.position + 1 > self.buffer.len() { return None }
		let value = self.buffer[self.position];
		self.position += 1;
		Some(value as i8)
	}
