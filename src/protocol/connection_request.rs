...
impl Packet for ConnectionRequest {
	...
	fn deserialize(buffer: &mut std::io::Cursor<Vec<u8>>) -> Option<Self> {
		...
		let guid = buffer.read_u64::<byteorder::LittleEndian>().unwrap();
		let timestamp = Duration::from_secs(buffer.read_u64::<byteorder::LittleEndian>().unwrap());
	...
	fn serialize(&self, buffer: &mut Vec<u8>) {
		...
		buffer.write_u64::<byteorder::LittleEndian>(self.guid).expect("Failed to write guid.");
		buffer.write_u64::<byteorder::LittleEndian>(self.timestamp.as_secs()).expect("Failed to write packet timestamp");
